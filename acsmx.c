		...
		int (*build_tree)(void * id, void **existing_tree) )
{
	...
	{
		for( mlist=acsm->acsmStateTable[i].MatchList; mlist!=NULL;
				mlist=mlist->next )
		{
			if (mlist->id)
			{
				build_tree(mlist->id, &acsm->acsmStateTable[i].MatchList->rule_option_tree);
			}
			cnt++;
		}
...
#define MAX_SEARCH_SIZE		(1024*1024)
		...
		void *data, int* current_state ) 
{
	...
	unsigned char Tc[MAX_SEARCH_SIZE];
	int state = 0;
	ACSM_PATTERN * mlist;
	unsigned char *Tend;
	...
	rn = (n < MAX_SEARCH_SIZE) ? n: MAX_SEARCH_SIZE;

	/* Case conversion */ 
	ConvertCaseEx (Tc, Tx, rn);
	T = Tc;
	Tend = T + rn;
